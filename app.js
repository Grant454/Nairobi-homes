...
const SUPABASE_URL  = 'https://xzxtcyyvetozzonckvzl.supabase.co';
const SUPABASE_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inh6eHRjeXl2ZXRvenpvbmNrdnpsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc4NDA2MTYsImV4cCI6MjA5MzQxNjYxNn0.GYuhRIr0z-EcJSsWaeCUYyfFkHdsE72yw61gIFZC0AA';
...
function _updateNav() {
  ...
  /* User area — must use display:flex not just remove hidden class */
  const userArea = $('nav-user-area');
  if (userArea) {
    userArea.style.display = logged ? 'flex' : 'none';
    userArea.classList.toggle('hidden', !logged);
  }

  /* Mobile user menu — must be flex when shown */
  const mobUser = $('mob-user');
  if (mobUser) {
    mobUser.style.display = logged ? 'flex' : 'none';
    mobUser.classList.toggle('hidden', !logged);
  }
...
async function doLogin() {
  ...
  try {
    ...
    /* ── Immediately load profile and update nav ────────────────
       Do NOT wait for onAuthStateChange — it fires asynchronously
       and the user would see the guest nav (Create Account button)
       until it resolves. We update the UI right now instead.
    ─────────────────────────────────────────────────────────── */
...
async function doRegister() {
  ...
  try {
    ...
    /* ── Immediately load profile and update nav ────────────────
       Same fix as doLogin — update the UI right now so the user
       sees their name instead of "Create Account" on landing.
    ─────────────────────────────────────────────────────────── */
    await _loadProfile(data.user);
    _updateNav();
